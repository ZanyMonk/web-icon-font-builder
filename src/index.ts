...
class IconFontBuildr {
  ...
  configInit(config?) {
    ...
    const DEFAULT_TEMPLATES = {
      css: [
        './templates/styles.css.ejs'
      ],
      scss: [
        './templates/_variables.scss.ejs',
        './templates/styles.scss.ejs'
      ]
    };
    ...
    this.configDefault = {
      ...
      output: {
        codepoints: false,
        ligatures: true,
        stylesheet: 'scss', // Can be truthy (CSS), 'css' or 'scss'
        templates: DEFAULT_TEMPLATES['scss'],
        icons: undefined,
    ...
    if (this.config.stylesheet && this.config.output.templates === undefined) {
      this.config.output.templates = this.config.stylesheet === 'scss' ? [
        path.join(__dirname, 'templates', 'styles.scss.ejs'),
        path.join(__dirname, 'templates', '_variables.scss.ejs')
      ] : [
        path.join(__dirname, 'templates', 'styles.css.ejs')
      ];

    }

    console.log('templates', this.config.output.templates);
  }
  ...
  getIconsStylesheet(): { [index: string]: string[] } {
    const data = {
      config: {
        mixinName: 'icons-font',
        prefix: 'icon-',
        fontName: this.config.output.fontName,
        fontsPath: path.relative(process.cwd(), this.config.output.fonts) + '/'
      },
      nonce: Math.random().toString(36).substr(2, 8),
      codepoints: this.getIconsCodepoints()
    };
    let results = {};

    this.config.output.templates.forEach(template => {
      let t = path.parse(template);
      ejs.renderFile(path.join(process.cwd(), template), data, (err, str) => {
        results[t.name] = str;
      });
    });
